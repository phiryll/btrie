...
func preOrder[T any](root T, adj func(T) iter.Seq[T]) iter.Seq[[]T] {
	return func(yield func([]T) bool) {
		preOrderRecurse(&stack[T]{root, nil, 1}, adj, yield)
	}
}

// Returns a value of true if done (some yield has returned false).
func preOrderRecurse[T any](s *stack[T], adj func(T) iter.Seq[T], yield func([]T) bool) bool {
	if !yield(s.path()) {
		return true
	}
	for node := range adj(s.value) {
		if preOrderRecurse(s.with(node), adj, yield) {
			return true
		}
	}
	return false
