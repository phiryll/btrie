...
func BenchmarkPreOrder(b *testing.B) {
	benchTraverser(b, btrie.TestingPreOrder)
}

func BenchmarkPostOrder(b *testing.B) {
	benchTraverser(b, btrie.TestingPostOrder)
}

func benchTraverser(b *testing.B, traverser btrie.TestingTraverser) {
	for _, tt := range []struct {
		...
		b.Run(tt.name, func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				for path := range traverser(0, tt.adj) {
					_ = path
				}
			}
		})
